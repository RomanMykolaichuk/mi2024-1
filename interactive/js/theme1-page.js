...
const lessonId=new URLSearchParams(window.location.search).get('lesson')||'t1-l1';
const allowed=new Set(['t1-l1','t1-l2','t1-l3','t1-l4','t1-l5']);
const root=document.querySelector('[data-role="lesson-root"]');

if(!allowed.has(lessonId)){
  root.innerHTML=`<div class="component-error" role="alert">Невідоме заняття Theme 1. <a href="../index.html">Повернутися на головну</a>.</div>`;
}else{
  try{
    ...
    console.error('[MI2024] Theme 1 shell',error);
...
function renderPage(lesson,source){
  return `
    <nav class="breadcrumb" aria-label="Навігація"><a href="../index.html">MI2024 Interactive</a><span>→</span><span>Тема 1 · ${esc(lesson.number)}</span></nav>
    ...
    <header class="hero hero--compact">
      ...
      <p class="eyebrow">ТЕМА 1 · ${esc(lesson.number)} · FOUNDATIONS</p>
    ...
    ${lesson.pipeline?`<section class="section-shell"><div class="section-heading"><p class="eyebrow">IAZ PIPELINE</p><h2>Місце заняття в аналітичному циклі</h2></div><div data-component="analytics-pipeline" data-source="${source}" data-select="pipeline"></div></section>`:''}
...
function renderTrack(current){
  const items=['1.1','1.2','1.3','1.4','1.5'];
  return `<nav class="track-nav" aria-label="Тема 1">${items.map(number=>`<a class="${number===current?'is-current':''}" href="theme1.html?lesson=t1-l${number.split('.')[1]}">${number}</a>`).join('')}</nav>`;
}
