...
const lessonId=new URLSearchParams(window.location.search).get('lesson')||'t5-l1';
const allowed=new Set(['t5-l1','t5-l2','t5-l3','t5-l4','t5-l5','t5-l6','t5-l7','t5-l8']);
const root=document.querySelector('[data-role="lesson-root"]');

if(!allowed.has(lessonId)){
  root.innerHTML=`<div class="component-error" role="alert">Невідоме заняття Theme 5. <a href="../index.html">Повернутися на головну</a>.</div>`;
}else{
  try{
    ...
    console.error('[MI2024] Theme 5 shell',error);
...
function renderPage(lesson,source){
  return `
    <nav class="breadcrumb" aria-label="Навігація"><a href="../index.html">MI2024 Interactive</a><span>→</span><span>Тема 5 · ${esc(lesson.number)}</span></nav>
    ...
    <header class="hero hero--compact">
      ...
      <p class="eyebrow">ТЕМА 5 · ${esc(lesson.number)} · VISUALIZATION</p>
    ...
    ${lesson.pipeline?`<section class="section-shell"><div class="section-heading"><p class="eyebrow">ANALYTICS PIPELINE</p><h2>Місце візуалізації в аналітичному процесі</h2></div><div data-component="analytics-pipeline" data-source="${source}" data-select="pipeline"></div></section>`:''}
...
function renderTrack(current){
  const items=['5.1','5.2','5.3','5.4','5.5','5.6','5.7','5.8'];
  return `<nav class="track-nav" aria-label="Тема 5">${items.map(number=>`<a class="${number===current?'is-current':''}" href="theme5.html?lesson=t5-l${number.split('.')[1]}">${number}</a>`).join('')}</nav>`;
}
