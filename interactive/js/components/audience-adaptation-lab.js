...
export function mount(element, config){
  ...
  function render(){
    ...
    options.innerHTML=`<p><strong>Які елементи залишити?</strong></p>${available.map(item=>`<button type="button" class="decision-option ${selections.has(item.id)?'is-selected':''}" data-element="${esc(item.id)}" aria-pressed="${selections.has(item.id)}">${esc(item.label)}</button>`).join('')}<div class="audience-fit" data-role="fit"></div>`;
    options.querySelectorAll('[data-element]').forEach(btn=>btn.addEventListener('click',()=>{
      const id=btn.dataset.element;
      selections.has(id)?selections.delete(id):selections.add(id);
      render();
    }));
    tabs.querySelectorAll('[data-audience]').forEach(btn=>btn.addEventListener('click',()=>{
