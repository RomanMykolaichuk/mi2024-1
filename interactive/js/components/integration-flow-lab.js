...
export function mount(element,config){
  ...
  function activeFlow(){return activeVariant()?.operations?.[operation]??[]}
  function reset(){selected=[];feedback.textContent='';renderMission()}
  function render(){
    ...
    operations.innerHTML=['GET','POST','DELETE'].filter(op=>v?.operations?.[op]).map(op=>`<button type="button" class="operation-tab ${op===operation?'is-selected':''}" data-operation="${op}">${op}</button>`).join('');
  ...
  function renderMission(){
    ...
    const poolOrder=activeVariant()?.poolOrder?.[operation]??flow.map(x=>x.id).slice().reverse();
    ...
    pool.querySelectorAll('[data-step]').forEach(btn=>btn.addEventListener('click',()=>{
      ...
      feedback.textContent=selected.length===flow.length?(activeVariant()?.complete?.[operation]??'Маршрут завершено: дані пройшли через чіткі контракти між UI, API і БД.'):`Наступний крок: ${flow[selected.length]?.label??''}`;
      renderMission();
    }));
  }
  render();
